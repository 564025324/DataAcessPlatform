...
$.widget("ech.multiselect", {
	...
	options: {
		...
		minWidth: 225,
		classes: '',
		checkAllText: /**'Check all'**/"全选",
		uncheckAllText: /**'Uncheck all'**/"全清",
		noneSelectedText: /**'Select options'**/"请选择",
	...
	_init: function(){
		...
		if(this.options.checkedvalue&&this.options.checkedvalue!=''){
			this.setCheckState(this.options.checkedvalue, true);
		}
	},
    ...
    setCheckState: function(checkedStr, flag){
        var values=checkedStr.split(';');
        if(values.length==0)
        	return false;        
        //toggle state on inputs
        ...
        $inputs.attr({ 'checked':flag, 'aria-selected':flag });
        this.update();
        //toggle state on original option tags
      this.element.find('option').filter(function(){
             return !this.disabled && $.inArray(this.value, values) > -1;
