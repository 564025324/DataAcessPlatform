...
(function() {
	...
	ol.loading=function(options) {
		...
		this.init=function(){
			...
			var mainHintDiv=$('<div class="ol_loading_div"></div>').css("z-index",options.zIndex+1);
			var loadingTextDiv=$('<div class="ol_loading_text" id="loadingText">'+ (!options.loadingText?'Loading...': options.loadingText) +'</div>').css("z-index",options.zIndex+1);
			var imageDiv=$('<div class="ol_loading"></div>').css("z-index",options.zIndex+1);
			mainHintDiv.append(loadingTextDiv);
			mainHintDiv.append(imageDiv);
			//alert(str);
			this.loadingImg=mainHintDiv;
			if(!options.loadingClass)
			{
				this.loadingImg.addClass(options.loadingClass);
				this.loadingMask.addClass(options.loadingClass+"_mask");
			}
      //this.loadingText=$('<div class="ol_loading_text" id="loadingText">'+ (!options.loadingText?'loading...': options.loadingText) +'</div>').css("z-index",options.zIndex+1);            
